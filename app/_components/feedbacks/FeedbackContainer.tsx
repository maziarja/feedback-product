import { ProductRequestsType, ProductRequestType } from "@/lib/types";
...
type FeedbackContainerProps = {
  ...
  isUpvoted: (upvotedBy: ProductRequestType["upvotedBy"]) => boolean;
};

async function FeedbackContainer({
  feedback,
  isUpvoted,
  ...
  const doesCurrentUserUpvote = isUpvoted(upvotedBy);
    ...
    >
      <div className="hidden @[688px]:block">
        <UpVoteButton
          upVotes={upVotes}
          feedbackId={_id}
          doesCurrentUserUpvote={doesCurrentUserUpvote}
        />
      </div>
      <div className="flex flex-col gap-2 text-sm">
        {feedback.status === "in-progress" && (
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="text-blue-gray text-sm md:text-base">In Progress</p>
          </div>
        )}
        {feedback.status === "planned" && (
          <div className="mb-3 flex items-center gap-2">
            <div className="bg-orange h-2 w-2 rounded-full" />
            <p className="text-blue-gray text-sm md:text-base">Planned</p>
          </div>
        )}
        {feedback.status === "live" && (
          <div className="mb-3 flex items-center gap-2">
            <div className="bg-light-blue h-2 w-2 rounded-full" />
            <p className="text-blue-gray text-sm md:text-base">Live</p>
          </div>
        )}
        <div className="space-y-1.5">
          ...
          <p className="text-dark-blue text-sm font-bold md:text-lg">{title}</p>
        </div>
        <p className="text-blue-gray text-sm whitespace-pre-wrap md:text-base @[688px]:mb-3">
          {description}
        </p>
      ...
      <div className="hidden cursor-pointer items-center gap-2 @[688px]:flex">
        ...
        <p
          className={`text-dark-blue text-sm font-bold md:text-base ${numOfComments === 0 ? "text-gray-400" : ""}`}
        >
          {numOfComments}
        </p>
      </div>
